...
import { eletrodomesticosProducts, type Product } from "@/lib/products-data"

const subcategories = ["Todos", "Geladeiras", "Micro-ondas", "Fogões", "Lavadoras", "Lava-louças", "Ar Condicionado"]
...
export default function EletrodomesticosPage() {
  ...
  const filteredAndSortedProducts = useMemo(() => {
    const filtered = eletrodomesticosProducts.filter((product) => {
  ...
  const availableBrands = useMemo(() => {
    return Array.from(new Set(eletrodomesticosProducts.map((product) => product.brand)))
  }, [])

  const maxPrice = useMemo(() => {
    return Math.max(...eletrodomesticosProducts.map((product) => product.price))
  }, [])
