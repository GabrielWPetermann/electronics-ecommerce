...
import { audioVideoProducts, type Product } from "@/lib/products-data"

const subcategories = ["Todos", "TVs", "Soundbars", "Fones de Ouvido", "Caixas de Som", "Home Theater", "Projetores"]
...
export default function AudioVideoPage() {
  ...
  const filteredAndSortedProducts = useMemo(() => {
    const filtered = audioVideoProducts.filter((product) => {
  ...
  const availableBrands = useMemo(() => {
    return Array.from(new Set(audioVideoProducts.map((product) => product.brand)))
  }, [])

  const maxPrice = useMemo(() => {
    return Math.max(...audioVideoProducts.map((product) => product.price))
  }, [])
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <nav className="text-sm text-muted-foreground">
              ...
              <span className="text-foreground">Áudio & Vídeo</span>
            </nav>
            <BackButton />
          </div>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <main className="flex-1">
            <div className="flex items-center justify-between mb-6">
              <div>
                <h1 className="text-3xl font-bold">Áudio & Vídeo</h1>
