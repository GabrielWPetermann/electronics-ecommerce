...
import { hardwareProducts, type Product } from "@/lib/products-data"
...
export default function HardwarePage() {
  ...
  const filteredAndSortedProducts = useMemo(() => {
    const filtered = hardwareProducts.filter((product) => {
  ...
  const availableBrands = useMemo(() => {
    return Array.from(new Set(hardwareProducts.map((product) => product.brand)))
  }, [])

  const maxPrice = useMemo(() => {
    return Math.max(...hardwareProducts.map((product) => product.price))
  }, [])
